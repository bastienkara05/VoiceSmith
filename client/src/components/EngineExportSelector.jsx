...
import styles from './EngineExportSelector.module.css'

export default function EngineExportSelector({ projectId, projectName, audioCount }) {
  ...
  const handleExport = async (engine) => {
    ...
    try {
      console.log(`Starting ${engine.label} export for project:`, projectId)

      const response = await fetch(`${engine.endpoint}/${projectId}`, {
        method: 'GET',
      })
